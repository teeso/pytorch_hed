...
class BSDS500(data.Dataset):
    ...
    def __init__(self,dataPath='/media/data2/zz/data/HED-BSDS', transform=None, target_transform=None):
        ...
        fin = open(os.path.join(dataPath, 'train_pair.lst'))
        self.image_list = []
    ...
    def __getitem__(self, index):
        ...
        label_path = os.path.join(self.dataPath, self.image_list[index]).replace('aug_data', 'aug_gt').replace('.jpg', '.png')
...
class BSDS500_TEST(data.Dataset):
    ...
    def __init__(self,dataPath='/media/data2/zz/data/HED-BSDS', transform=None):
        super(BSDS500_TEST, self).__init__()
        # list all images into a list
        # self.image_list = [x for x in listdir(dataPath) if is_image_file(x)]
        fin = open(os.path.join(dataPath, 'test.lst'))
        self.image_list = [_.rstrip() for _ in fin.readlines()]
        fin.close()
        self.dataPath = dataPath
        self.transform = transform
